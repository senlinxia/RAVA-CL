...
def resnet50(out_dim):
    ...
    """
    return ResNet(BottleNeck, [3, 4, 6, 3],out_dim)
...
class ResNetRAVA_CL(nn.Module):
    def __init__(self, out_dim):
        super(ResNetRAVA_CL, self).__init__()
        # self.backbone=resnet18(out_dim)
        self.backbone = resnet50(out_dim)
...
class ResNetSimCLRWith(nn.Module):
    def __init__(self,out_dim,cls):
        super(ResNetSimCLRWith, self).__init__()
        ...
        self.linear=nn.Linear(dim_mlp, cls)
...
if __name__ == '__main__':
    ...
    x=torch.rand(16,1,128)
    y=model(x)
    print(y.shape)

    model=ResNetRAVA_CL(2048)
    y=model(x)
    print(y.shape)
